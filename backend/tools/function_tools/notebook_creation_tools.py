        ...
        async def _create_notebook():
            ...
            try:
                ...
                if not hasattr(creator_agent, 'sections') or not creator_agent.sections:
                    error_msg = "错误：notebook_agent_creator 未能生成内容。请检查 agent 是否正确执行。"
                    if hasattr(creator_result, 'final_output'):
                        error_msg += f"\nAgent 输出: {str(creator_result.final_output)[:500]}"
                    if hasattr(creator_result, 'messages') and creator_result.messages:
                        last_msg = creator_result.messages[-1] if creator_result.messages else None
                        if last_msg and hasattr(last_msg, 'content'):
                            error_msg += f"\n最后一条消息: {str(last_msg.content)[:500]}"
                    print(f"[create_notebook_with_outline] {error_msg}")
                    return error_msg
                
                sections = creator_agent.sections  # Dict[str, Section]
                print(f"[create_notebook_with_outline] 成功生成 {len(sections)} 个章节")
...
def create_handle_file_upload_tool(top_level_agent: 'BaseAgent'):
    ...
    """
    from backend.tools.utils import save_uploaded_file
    from backend.tools.tool_registry import get_tool_registry
    from agents import Runner
    import json
        ...
        async def _generate_outline():
            ...
            try:
                ...
                if not file_path or not file_path.strip():
                    return "错误：file_path 参数为空。如果用户没有上传文件，请使用 generate_outline 工具而不是 handle_file_upload 工具。"
                ...
                if not os.path.isfile(stored_path):
                    return f"错误: 路径不是文件（可能是目录）: {stored_path}\n\n如果用户没有上传文件，请使用 generate_outline 工具而不是 handle_file_upload 工具。"
                    ...
                    try:
                        ...
                        if not hasattr(creator_agent, 'sections') or not creator_agent.sections:
                            ...
                            if hasattr(creator_result, 'messages') and creator_result.messages:
                                ...
                                if last_msg and hasattr(last_msg, 'content'):
                                    error_msg += f"\n最后一条消息: {str(last_msg.content)[:500]}"
                            print(f"[create_notebook_from_outline] {error_msg}")
                            return error_msg
                        
                        sections = creator_agent.sections  # Dict[str, Section]
                        print(f"[create_notebook_from_outline] 成功生成 {len(sections)} 个章节")
