...
function ChatPage() {
  ...
  // 图片处理固定流程状态
  const [questionImage, setQuestionImage] = useState(null) // { file: File, preview: string, base64: string }
  const [questionText, setQuestionText] = useState(null) // 题目原文
  const [questionAnswer, setQuestionAnswer] = useState(null) // 答案
  const [questionHint, setQuestionHint] = useState(null) // 提示
  const [generatingText, setGeneratingText] = useState(false) // 正在生成原文
  const [gettingAnswer, setGettingAnswer] = useState(false) // 正在获取答案
  const [gettingHint, setGettingHint] = useState(false) // 正在获取提示
  const [addingToNotebook, setAddingToNotebook] = useState(false) // 正在添加到笔记
  ...
  // 生成题目原文
  const handleGenerateQuestionText = async () => {
    ...
    try {
      ...
      const images = [{
        type: "input_image",
        detail: "auto",
        image_url: questionImage.base64,
      }]
      ...
      setQuestionText(response.data.response)
    } catch (err) {
      console.error('生成题目原文失败:', err)
      setError(`生成题目原文失败: ${err.message}`)
    } finally {
  ...
  const handleGetAnswer = async () => {
    ...
    try {
      ...
      setQuestionAnswer(response.data.response)
    } catch (err) {
  ...
  const handleGetHint = async () => {
    ...
    try {
      ...
      setQuestionHint(response.data.response)
    } catch (err) {
  ...
  const handleAddToNotebook = async () => {
    ...
    try {
      ...
      // 使用chatWithTopLevelAgent创建笔记
      const response = await chatWithTopLevelAgent(
        `请根据以下内容创建一个笔记：\n\n${notebookContent}\n\n请调用 handle_file_upload 工具，将内容保存为markdown文件并创建笔记本。`,
        sessionId,
        null
      )
                  ...
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    ...
                    {questionText && (
                      <>
                        ...
                        >
                          {gettingHint ? '获取中...' : '得到提示'}
                        </Button>
                        {(questionAnswer || questionHint) && (
                          <Button
                            variant="contained"
                            size="small"
                            ...
                            sx={{
                              bgcolor: isDark ? '#ab68ff' : '#5856D6',
                              '&:hover': {
                                bgcolor: isDark ? '#8b4fd9' : '#4a3db3',
                              },
                            }}
                          >
                            {addingToNotebook ? '添加中...' : '添加到笔记'}
                          </Button>
                        )}
                      </>
                    )}
